...
# Simple switch configuration script for network engineers
# This helps manage switch configs without complex coding
"""
...
class Switch:
    ...
    def configure_port(self, interface, vlan=None, mode="access", description=""):
        ...
        if mode == "access" and vlan:
            ...
            # Remove port from old VLAN first
            old_vlan = port.get("vlan")
            if old_vlan and old_vlan in self.vlans:
                if interface in self.vlans[old_vlan]["ports"]:
                    self.vlans[old_vlan]["ports"].remove(interface)
            ...
            # Add to new VLAN
            if interface not in self.vlans[vlan]["ports"]:
    ...
    def check_ip_format(self, ip):
        """Simple IP validation for network engineers"""
        try:
            parts = ip.split('.')
            if len(parts) != 4:
                return False
    ...
    def generate_config(self):
        ...
        if not self.check_ip_format(self.management_ip):
            print(f"❌ Bad IP address: {self.management_ip}")
            return None
...
"""
        ...
        try:
            ...
            print(f"❌ Can't save file - check permissions!")
        except:
            print(f"❌ File save error")
            
        return config

# Demo section - shows how to use the switch class
...
class IntelligentSwitch(Switch):
    ...
    def auto_configure_port_by_device(self, interface, device_type):
        ...
        if device_type.lower() in device_configs:
            ...
            # Configure the port
            if config.get("mode") == "trunk":
                # For trunk ports (like AP)
                self.configure_trunk_port(interface, native_vlan=1)
                self.interfaces[interface]["description"] = config["description"]
            else:
                # For access ports
                self.configure_port(
                    interface,
    ...
    def security_audit(self):
        ...
        # Check for unused ports - security risk!
        unused_ports = []
        for intf, settings in self.interfaces.items():
            if settings["status"] == "down":
                unused_ports.append(intf)
        ...
        # Check for default VLAN 1 usage - bad practice!
        default_vlan_ports = []
        for intf, settings in self.interfaces.items():
            if settings.get("vlan") == 1 and settings["status"] == "up" and settings["mode"] == "access":
                default_vlan_ports.append(intf)
    ...
    def optimize_config(self):
        ...
        for interface, settings in self.interfaces.items():
            if settings["status"] == "up":
                ...
                vlan = settings.get("vlan", "none")
                mode = settings.get("mode", "access")
        ...
        for (vlan, mode, description), interfaces in config_groups.items():
            if len(interfaces) > 1:
...
if __name__ == "__main__":
    ...
    print("\n✅ Script complete! Check your config files.")